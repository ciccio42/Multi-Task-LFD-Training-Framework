#!/bin/bash
# Policy finetune for the rm_one_spawn spawn-region experiment: trains a CondPolicy
# (multi_task_il.models.cond_target_obj_detector.policy.CondPolicy) with a frozen, pretrained
# CondTargetObjectDetector backbone (see train_cond_target_obj_detector_rm_one_spawn.sh),
...
# adapted to this cluster's paths/conda env and to the rm_one_spawn spawn-restricted
# trajectory manifest (same TRAJ_MANIFEST as the detector training).
#
# Detector checkpoint selection: swept model_save-{12,24,36,48}.pt with
# test_cod_spawn_region_variants.sh <variant> <epoch> test_gt (real recorded trajectories +
# real human_rgb demo context, training-scenario only) -- this variant had no prior test
# coverage before this sweep. Detection quality (TP/FP/FN):
#   ep12: TP 0.579, FP 0.401, FN 0.005
#   ep24: TP 0.619, FP 0.305, FN 0.061  <- selected (highest TP)
#   ep36: TP 0.546, FP 0.394, FN 0.045
#   ep48: TP 0.388, FP 0.547, FN 0.050
# Milder, steadier decline than rm_central_spawn (no sharp late collapse observed up to
# epoch 48), but still clearly past its peak by epoch 48 -- epoch 24 selected as backbone.
...
# then submit with: sbatch train_cond_target_obj_detector_policy_rm_one_spawn.sh
...
#SBATCH --export=ALL
#SBATCH --job-name=cod_policy_rm_one_spawn
#SBATCH --output=slurm_cod_policy_rm_one_spawn_%j.out
...
TRAJ_MANIFEST=/mnt/beegfs/frosa/Multi-Task-LFD-Framework/repo/open_x_embodiment/datasets/ur5e_pick_place_rm_one_spawn/new_trj_removed_spawn_regions.json
POLICY='${cond_policy}'
DATASET_TARGET=multi_task_il.datasets.multi_task_cond_target_obj_dataset.CondTargetObjDetectorDataset

EXP_NAME=1Task-pick_place-COD_rm_one_spawn-Policy
...
COND_TARGET_OBJ_DETECTOR_WEIGHTS="/mnt/beegfs/frosa/checkpoint_save_folder/checkpoint_save_folder/spawn_region_experiments/1Task-pick_place-COD_rm_one_spawn-Batch80"
COND_TARGET_OBJ_DETECTOR_STEP=24
