...
# ur5e_pick_place_removed_spawn_regions/ (see that dir's new_trj_removed_spawn_regions.json, and
# multi_task_il/datasets/utils.py's create_train_val_dict()'s new trajectory_manifest param this
# points dataset_cfg.trajectory_manifest at).
#
# Unlike the reference run, ALL 16 pick_place subtasks are used in training here
# (tasks_cfgs.pick_place.skip_ids=[] override, vs. the reference's default [12,13,14,15]) - the
# restriction is WHICH trajectories within each task_XX folder are read (spawn position of the
# object), not which tasks. Since set_same_n=7 is unchanged and train_any.py recomputes
# cfg.bsize = set_same_n * (n_tasks - len(skip_ids)) (train_any.py:61-62), the effective batch
# size becomes 7*16=112 here (vs the reference's 7*12=84) purely as a consequence of training on
# all 16 tasks instead of 12 - not manually tuned.
#
# EXPERT_DATA/save_path corrected to the real beegfs paths (the original script's
# /home/rsofnc000/dataset and /home/rsofnc000/checkpoint_save_folder don't exist on this login
# node - /home/rsofnc000 has no dataset/ subdir at all, confirmed empty of any COD checkpoints).

#SBATCH -A did_robot_learning_359
#SBATCH --partition=gpuq
# Whole-node exclusive: this run stalled at epoch 6 for multiple 7h cycles with zero
# progress because it kept landing co-located with other COD/unrelated jobs on gnode01,
# whose dataloader workers (num_workers=32 each, no OMP/MKL/cv2 thread pinning)
# oversubscribed the node's 128 CPUs far beyond what SLURM's own cpus-per-task
# accounting showed. --exclusive guarantees no other job shares this node's CPUs,
# at the cost of possibly queueing until a fully-idle node opens up.
#SBATCH --exclusive
...
#SBATCH --export=ALL
#SBATCH --job-name=cod_removed_spawn_regions
#SBATCH --output=slurm_cod_removed_spawn_regions_%j.out
