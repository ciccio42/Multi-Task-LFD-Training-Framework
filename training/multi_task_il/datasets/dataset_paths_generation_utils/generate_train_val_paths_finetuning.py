...
TASK_LIST = ['pick_place', 'nut_assembly', 'stack_block', 'button']
...
def main():
    ...
    parser.add_argument('--ur5e_sim_folder')
    ...
    parser.add_argument("--human_dataset", action='store_true')
    ...
    # ur5e_sim_path = '/user/frosa/multi_task_lfd/ur_multitask_dataset/opt_dataset/pick_place/panda_pick_place'
    ...
    if args.panda_sim_dataset:
        for task in TASK_LIST:
            ...
            panda_folder = os.path.join(args.dataset_folder, task, f"panda_{task}")
            for root, dirs, files in os.walk(panda_folder):
                # print(root)
                # print(dirs)
                # print(files)
                # print('\n')
                if len(dirs) != 0 and root.split('/')[-1] == f"panda_{task}":
                    dataset_name = root.split('/')[-1]
                    for pkl_dict in pkl_files_paths:
    ...
    if args.ur5e_sim_dataset:
        print(f'searching into {args.ur5e_sim_folder}')
        for root, dirs, files in os.walk(args.ur5e_sim_folder):
            # print(root)
            # print(dirs)
            # print(files)
            # print('\n')
            if len(dirs) != 0 and root.split('/')[-1] == args.ur5e_sim_folder.split('/')[-1]:
                dataset_name = root.split('/')[-1]
                for pkl_dict in pkl_files_paths:
                    pkl_dict[dataset_name] = {}
    ...
    if args.human_dataset:
        print(f'searching into {args.human_dataset_folder}')
        for root, dirs, files in os.walk(args.human_dataset_folder):
            if len(dirs) != 0 and root.split('/')[-1] == args.human_dataset_folder.split('/')[-1]:
                dataset_name = root.split('/')[-1]
                for pkl_dict in pkl_files_paths:
                    pkl_dict[dataset_name] = {}
