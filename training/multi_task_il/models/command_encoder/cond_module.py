...
def get_backbone(backbone_name="slow_r50", video_backbone=True, pretrained=True, conv_drop_dim=3):
    if video_backbone:
        ...
        if backbone_name == "r2plus1d_18":
            if not pretrained:
                ...
                print("Loading pretrained weights for r2plus1d_18")
...
class CondModule(nn.Module):
    ...
    def __init__(self, height=120, width=160, demo_T=4, model_name="slow_r50", pretrained=True, cond_video=True, n_layers=3, demo_W=7, demo_H=7, demo_ff_dim=[128, 64, 32], demo_linear_dim=[512, 256, 128], conv_drop_dim=3):
        super().__init__()
        self._demo_T = demo_T
